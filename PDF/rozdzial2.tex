...
\begin{enumerate}
	...
	\begin{enumerate}
		\item metoda MIP
		\item naiwna - zaokrągalanie rozwiązania zwróconego przez metodę simpleksową
	\end{enumerate}
...
Teraz pozostaje, tak opisany w programie, problem rozwiązć. 
Inicjalizajca obydwu metod (MIP i nawinej) do tego momentu przebiegała tak samo (z tym, że w tej drugiej nie trzeba nakładać ograniczenia calkowitoliczbowego na zmienne). 

\subsubsection{MIP}
GLPK do rozwiązywania problemów MIP używa solvera opartego na metodzie \textit{branch-and-cut}, która jest hybrydą metod \textit{branch-and-bound} i \textit{cutting plane}. 
...
W metodzie naiwnej nie trzeba nakładać ograniczeń na typ zmiennych (\ref{typ_zmiennych}). Sam proces jest ma wspó
